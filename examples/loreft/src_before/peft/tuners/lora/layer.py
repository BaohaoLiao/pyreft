...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        if self.is_feedforward:
            self.lora_A[adapter_name] = nn.Linear(self.in_features//2, 1, bias=False)
            self.lora_B[adapter_name] = nn.Linear(self.in_features//2, 1,  bias=False)
        else:
    ...
    def _apply_rosa(self, x, lora_A, lora_B):
        ...
        lora_A_sin = lora_A.weight.squeeze().repeat_interleave(2).cos()
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        LoraLayer.__init__(self, base_layer, is_feedforward=is_feedforward, **kwargs)
