...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "lora_rosa_A", "lora_rosa_B", "lora_embedding_A", "lora_embedding_B")
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.lora_rosa_A = nn.ModuleDict({})
        self.lora_rosa_B = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        self.lora_rosa_A[adapter_name] = nn.Linear(self.out_features//2, 1, bias=False)
        self.lora_rosa_B[adapter_name] = nn.Linear(self.out_features//2, 1, bias=False)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            if init_lora_weights is True:
                ...
                nn.init.zeros_(self.lora_rosa_A[adapter_name].weight)
                nn.init.ones_(self.lora_rosa_B[adapter_name].weight)
    ...
    def _apply_rosa(self, x, rosa_A, rosa_B):
        rosa_A_cos = rosa_A.weight.squeeze().repeat_interleave(2).cos()
        rosa_A_sin = rosa_A.weight.squeeze().repeat_interleave(2).sin()
        rosa_B_repeat = rosa_B.weight.squeeze().repeat_interleave(2)

        rosa_A_cos = rosa_B_repeat * rosa_A_cos
        rosa_A_sin = rosa_B_repeat * rosa_A_sin
    
        rotate_half_x = torch.stack([-x[..., 1::2], x[..., ::2]], dim=-1).reshape_as(x)
        x = x * rosa_A_cos + rotate_half_x * rosa_A_sin
        return x
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in self.lora_A.keys():
                    continue
                lora_rosa_A = self.lora_rosa_A[active_adapter]
                lora_rosa_B = self.lora_rosa_B[active_adapter]
                lora_A = self.lora_A[active_adapter]
                lora_B = self.lora_B[active_adapter]
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                ...
                result = result.to(lora_rosa_A.weight.dtype)

                if not self.use_dora[active_adapter]:
                    result = self._apply_rosa(result, lora_rosa_A, lora_rosa_B)
