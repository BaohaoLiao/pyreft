...
class ClassificationHead(nn.Module):
    def __init__(self, config):
        ...
        self.type = config.classification_type
        assert self.type in ["original", "no_norm", "no_angle"]

    def _get_weight_norm(self, weight):
        eps = 1e-4
        out = torch.sqrt(torch.sum(weight * weight, dim=[1]) + eps)
        return out.unsqueeze(0)

    def _get_input_norm(self, feat):
        eps = 1e-4
        input_norm = torch.sqrt(torch.sum(feat*feat, dim=[-1], keepdim=True) + eps)
        return input_norm
    ...
    def forward(self, features, **kwargs):
        ...
        elif self.type == "no_norm":
            weight = self.dense.weight
            xnorm = self._get_input_norm(x)
            wnorm = self._get_weight_norm(weight)
            x = self.dense(x)
            x = x / xnorm
            x = x / wnorm
            x = torch.tanh(x)
...
def main():
    ...
    # Load model
    assert data_args.task in classification_tasks
