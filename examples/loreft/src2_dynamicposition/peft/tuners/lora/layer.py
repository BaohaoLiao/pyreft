...
class LoraLayer(BaseTunerLayer):
    ...
    def _apply_rosa(self, x, lora_A, lora_B, positions):
        assert len(x.size()) == 3
        lora_A_w1 = lora_A.weight[0]
        lora_A_w2 = lora_A.weight[1]
        lora_B_w1 = lora_B.weight[0]
        lora_B_w2 = lora_B.weight[1]

        position_ids = positions.weight.squeeze()[:x.size(1)]
        lora_A_w1 = torch.outer(position_ids, lora_A_w1)
        lora_A_w2 = torch.outer(position_ids, lora_A_w2)
        lora_A_w_sin = lora_A_w2.sin() * lora_B_w2.unsqueeze(0)
        lora_A_w_cos = lora_A_w1.cos() * lora_B_w1.unsqueeze(0)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    result = self._apply_rosa(result, lora_A, lora_B, positions)
                else:
                    x = dropout(x)
